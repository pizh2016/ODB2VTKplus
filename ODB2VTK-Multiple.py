...
def PartitionPointCenterSerial(config = {}, node_coordination = [], element = ''):
	...
	for i_element in element:
		label = i_element.label
		element_connectivity[label - 1] = i_element.connectivity
		tempcoordinate = (node_coordination[i_element.connectivity[elementnode] - 1]).sum() / mesh_conner
		for i in range(piecenum):
...
def WriteToVTKSerial(config = {}, frame = '', newmodel = {}, node_coordination = [], stepname = '', instancename = '',  max_nodelabel = 0, instance_region = None):
	...
	for i_frame in frame_list:
		...
		if vector_amount > 0 and u'' not in vectorlist:
			for vector_name in vectorlist:
				tempvector = ReadVector(n_frame.fieldOutputs[vector_name].getSubset(region=instance_region),max_nodelabel)
				vector_dict[vector_name] = copy.deepcopy(tempvector)
		else:
			print "No Vector to access! check config file!"
		...
		# Stress components
		if "S" in tensorlist:
			tensor_dict["S"] = ReadTensorAtIntS(n_frame.fieldOutputs["S"].getSubset(region=instance_region),7,max_nodelabel)
		# Logarithmic strain components
		if "LE" in tensorlist:
			tensor_dict["LE"] = ReadTensorAtIntE(n_frame.fieldOutputs["LE"].getSubset(region=instance_region),2,max_nodelabel)
		# Plastic strain components
		if "PE" in tensorlist:
			tensor_dict["PE"] = ReadTensorAtIntE(n_frame.fieldOutputs["PE"].getSubset(region=instance_region),2,max_nodelabel)
		print "Time elapsed: ", time() - time1, "s"
		
		# Access Saclar: PEEQ
		if config['output_scalar']==[""]:
			saclarlist = []
			scalar_dict = {}
		else:
		...
		if "PEEQ" in saclarlist:
			time1 = time()
			print "Reading "+"PEEQ"+"......"
			scalar_dict["PEEQ"] = ReadScalarAtInt(n_frame.fieldOutputs["PEEQ"].getSubset(region=instance_region),max_nodelabel)
			print "Time elapsed: ", time() - time1, "s"	
		...
		for pn in piecekey:
			...
			XYZ = node_coordination[newnode,:]+tempvector[newnode,:]
			...
			for tensorkey in tensorlist:
				...
				XYZTENSOR = temptensor[newnode,:]
				for i in newnodeamount_N:
					outfile.write(' {:11.8e}  {:11.8e}  {:11.8e}  {:11.8e}  {:11.8e}  {:11.8e}  {:11.8e}  {:11.8e}  {:11.8e}\n'.format(*XYZTENSOR[i,[0,3,5,3,1,4,5,4,2]]))
			...
			for tensorkey in tensorlist:
				...
				for invariantkey in tensor_dict[tensorkey]['validinvariant']:
					...
					X = invariant[newnode,i_validinvariant]
					for i in newnodeamount_N:
						outfile.write(' {:11.8e}\n'.format(X[i]))
					outfile.write('</DataArray>'+'\n')
		...
		if ( config['piece_number'] > 1 ):
			...
			time1 = time()
			outfile = open (os.path.join(config['vtk_path'],config['odbname'])+'_'+stepname+'_'+instancename+'_'+'f%03d'%int(i_frame)+'.pvtu','w')
...
def WriteToVTKParallel(config = {}, newmodel = {}, node_coordination = [], stepname = '', instancename = '', max_nodelabel = 0):
	...
	print "Write to VTK files by multiprocessing......"
	processlist = []
	for i in range(process_munber):
...
def WriteToVTKSubFunction(config, newmodel, node_coordination, stepname, instancename, framelist, max_nodelabel):
	...
	piecenum = config['piece_number']
	instance_region = odb.rootAssembly.instances[instancename]
...
#Read from each .pvtu and write to assembly .pvtu file
def VTKAssembly(config):
	#====================================组装.pvtu文件=======================================#	
	return
#------------Read from each .pvtu and write to assembly .pvtu file----------#
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
...
def ConvertOdb2Vtk(filename = 'ODB2VTK+.cfg'):
	...
	# check vtk_path exists
	if not os.path.exists(config['vtk_path']):
		os.makedirs(config['vtk_path'])
	...
	for i in config['input_instance']:
		if(instance_amount < int(i) or instance_amount == int(i)):
	...
	for i_instance in config['input_instance']:
		...
		if(config['parallel_partition'] == 'TRUE'):
			...
			if(config['piece_number'] == 1):
				# no Partition
				print "no partition, deal with whole part"
				piecedict={'000': np.zeros(0, dtype=np.int32)}
				element_connectivity = np.zeros((element_amount,config['mesh_conner']),dtype=np.int32)
				for i_element in element:
					label = i_element.label
					element_connectivity[label - 1] = i_element.connectivity
					piecedict['000'] = np.append(piecedict['000'], label)
			else:
		...
		#===================================================================================#
		# Step cycle
		# define region for loop
		instance_region = instance[instancename]
		...
		for i_step in config['input_step']:
			...
			if(config['parallel_frame'] == 'TRUE'):
				...
				WriteToVTKSerial(config, frame, newmodel, node_coordination, stepname, instancename, max_nodelabel, instance_region)
			else:
